...
export function WalletModal({ show, onClose, walletForm, setWalletForm, editingWallet, onSave }) {
  if (!show) return null;
  const colors = ['#10b981','#6366f1','#f59e0b','#f43f5e','#8b5cf6','#0ea5e9','#14b8a6','#ec4899'];
  const types = [{ val:'cash', label:'Tunai' },{ val:'bank', label:'Bank' },{ val:'ewallet', label:'E-Wallet' },{ val:'card', label:'Kartu' }];
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md overflow-hidden">
        ...
        <div className="p-5 space-y-4">
              ...
              {types.map(t => (
                <button key={t.val} type="button" onClick={() => setWalletForm({ ...walletForm, type: t.val })}
                  className={`py-3 rounded-xl border-2 text-xs font-bold transition-all ${
                    walletForm.type === t.val ? 'border-emerald-400 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500 hover:border-slate-300'
                  }`}>
                  {t.label}
                </button>
              ))}
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-2xl text-xs font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all">Batal</button>
            <button type="button" onClick={onSave} className="flex-1 py-3 rounded-2xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-700 transition-all shadow-lg">
