...
import {
  ...
  faCalendarDays, faSliders, faChevronUp
...
function walletTypeLabel(type) {
  const map = { cash: 'Tunai', bank: 'Bank', ewallet: 'E-Wallet', card: 'Kartu Kredit' };
...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (txData.user?.name) {
        setUserName(txData.user.name);
        localStorage.setItem('ft_username', txData.user.name);
      }
    } catch {
  ...
  const SummaryCards = () => (
    <div className="grid grid-cols-3 gap-2 sm:gap-4">
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white p-3 sm:p-5 rounded-2xl shadow-lg shadow-emerald-500/20 overflow-hidden relative cursor-default">
        <div className="absolute -right-3 -bottom-3 text-6xl opacity-[0.08]"><FontAwesomeIcon icon={faArrowTrendUp} /></div>
      ...
      <div className="bg-gradient-to-br from-rose-500 to-pink-600 text-white p-3 sm:p-5 rounded-2xl shadow-lg shadow-rose-500/20 overflow-hidden relative cursor-default">
        <div className="absolute -right-3 -bottom-3 text-6xl opacity-[0.08]"><FontAwesomeIcon icon={faArrowTrendDown} /></div>
      ...
      <div className={`text-white p-3 sm:p-5 rounded-2xl shadow-lg overflow-hidden relative cursor-default transition-all duration-500 ${dynamicSaldo >= 0 ? 'bg-gradient-to-br from-indigo-600 to-blue-700 shadow-indigo-500/20' : 'bg-gradient-to-br from-orange-500 to-red-600 shadow-orange-500/20'}`}>
        <div className="absolute -right-3 -bottom-3 text-6xl opacity-[0.08]"><FontAwesomeIcon icon={faScaleBalanced} /></div>
        <p className="text-[9px] sm:text-xs font-bold opacity-70 uppercase tracking-widest mb-1.5">{isFiltered ? 'Selisih' : 'Saldo'}</p>
        <h3 className="text-xs sm:text-lg font-black leading-tight">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={openFilterModal}
              className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition-all ${activeFilterCount > 0 ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}>
              <FontAwesomeIcon icon={faSliders} />
              <span className="hidden sm:inline">Filter</span>
              {activeFilterCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 h-4 w-4 bg-emerald-500 text-white text-[9px] font-black rounded-full flex items-center justify-center">{activeFilterCount}</span>
              )}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <header className="bg-white border-b border-slate-200/60 px-4 sm:px-6 py-0 sticky top-0 z-30 shadow-sm">
        <div className="max-w-2xl mx-auto flex justify-between items-center h-14 sm:h-16">
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={userMenuRef}>
              <button onClick={() => setUserMenuOpen(o => !o)}
                ...
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-bold text-slate-800 leading-tight max-w-[80px] truncate">{userName || 'Pengguna'}</p>
                </div>
                <FontAwesomeIcon icon={faChevronDown} className={`text-[10px] text-slate-400 transition-transform duration-200 ${userMenuOpen ? 'rotate-180' : ''}`} />
              </button>

              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden z-50 animate-in fade-in slide-in-from-top-1 duration-150">
      ...
      <div id="sidebar"
        className={`hidden md:block fixed top-[57px] sm:top-[65px] left-0 h-[calc(100vh-65px)] w-64 bg-white border-r border-slate-200 z-20 transition-transform duration-300 shadow-xl shadow-slate-200/60 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      {sidebarOpen && (
        <div className="hidden md:block fixed inset-0 bg-slate-900/20 z-10 top-[65px]" onClick={() => setSidebarOpen(false)} />
      )}

      <main className="max-w-2xl mx-auto px-4 sm:px-5 pt-5 pb-24 md:pb-10">
        <SummaryCards />
        <div className="mt-6">
            ...
            return (
              <button key={tab.id} onClick={() => navigateTo(tab.id)}
                ...
                <div className={`flex items-center justify-center w-7 h-7 rounded-xl transition-all ${isActive ? 'bg-emerald-50' : ''}`}>
                  <FontAwesomeIcon icon={tab.icon} className={`text-base transition-all ${isActive ? 'text-emerald-600' : 'text-slate-400'}`} />
                </div>
                <span className={`text-[9px] font-bold leading-none transition-all ${isActive ? 'text-emerald-600' : 'text-slate-400'}`}>{tab.label}</span>
              </button>
            );
          })}
        </div>
      ...
      {activeTab !== 'dashboard' && (
        <button onClick={() => setShowModal(true)}
          className="md:hidden fixed bottom-[72px] right-4 h-13 w-13 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl shadow-xl shadow-emerald-500/40 flex items-center justify-center transition-all active:scale-95 z-30"
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={() => setShowFilterModal(false)} className="h-9 w-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all">
                <FontAwesomeIcon icon={faXmark} />
              </button>
            </div>
            <div className="p-5 space-y-5">
              ...
              <div>
                ...
                {tempFilterMonth && (
                  <button onClick={() => setTempFilterMonth('')} className="mt-1.5 text-xs text-rose-500 font-bold">Hapus pilihan bulan</button>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" placeholder="Cari deskripsi atau kategori..." value={tempSearch} onChange={e => setTempSearch(e.target.value)}
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={resetFilter} className="flex-1 py-3 rounded-2xl text-xs font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all">
                  Reset
                </button>
                <button onClick={applyFilter} className="flex-[2] py-3 rounded-2xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-700 transition-all shadow-lg">
                  Terapkan Filter
                </button>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[
                    ...
                    { val:'card', label:'Kartu' }
                  ].map(t => (
                    <button key={t.val} type="button" onClick={() => setWalletForm({ ...walletForm, type: t.val })}
                      className={`py-3 rounded-xl border-2 text-xs font-bold transition-all ${walletForm.type === t.val ? 'border-emerald-400 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
